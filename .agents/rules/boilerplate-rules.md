...
6. **Aşırı Küçük Font Size (`text-xs`, `text-[10px]`, `text-[11px]`, `text-[12px]`) Kullanma:** Projede rozetler, etiketler, ürün kodları, ikincil açıklamalar veya butonlar dahil hiçbir yerde `text-xs` (12px) veya daha küçük font boyutları **kullanılamaz**. Tüm etiketler, rozetler ve okunabilir metinler en az `text-sm` (14px) veya `text-base` (16px) font boyutunda olmalıdır.
