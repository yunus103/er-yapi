...
type SitemapData = {
  pages?: Record<"home" | "about" | "contact" | "blog" | "products" | "services" | "projects", SitemapPage | null>;
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const staticRouteEntries: MetadataRoute.Sitemap = [
    ...
    { url: `${base}/urunler`, lastModified: lastModified(pages?.products?._updatedAt), changeFrequency: "daily", priority: 0.9 },
    ...
    { url: `${base}/blog`, lastModified: lastModified(pages?.blog?._updatedAt), changeFrequency: "weekly", priority: 0.8 },
  ];

  const staticRoutes = staticRouteEntries.filter((route) => {
    const path = route.url.replace(base, "") || "/";
    if (path === "/") return !pages?.home?.noIndex;
    if (path === "/urunler") return !pages?.products?.noIndex;
